...
class Weather extends Component {
  ...
  async componentDidMount () {
    Geolocation.getCurrentPosition( 
      (position) => { this.setState({lat: position.coords.latitude, long: position.coords.longitude, coords: position.coords})
  ...
  setTimeout(()=>{console; }, 4000)

//   alert(this.state.lat + " LAT" + this.state.long);
const API_KEY='e37662e2508a1d950408d6a6ebb47251';
// const crd = position.coords;  console.log('Your current position is:'); console.log(`Latitude : ${crd.latitude}`); // console.log(`Longitude: ${crd.longitude}`);  console.log(`More or less ${crd.accuracy} meters.`);
// const api_call = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${position.coords.latitude}&lon=${position.coords.longitude}&appid=${API_KEY}`);
...
if (response.cod ==="4000") {alert('snap')}

      else {let TempInF = await response?.main?.temp * (9/5) - 459.67;
        let wind = await response?.wind?.speed;
      //   console.log("wind: " +wind);
     let shortTempInF = TempInF.toFixed(2);
      let windVar = await response.wind.speed*2.23694;
...
// windRound = this.state.windspeed; let windFixed= windRound.toFixed(2); console.log(windRound.toFixed(2));
...
const styles = StyleSheet.create({
    ...
    container: {
      ...
      textInputStyle:{textAlign: 'center', marginTop: 40, backgroundColor: 'lightgrey'},
